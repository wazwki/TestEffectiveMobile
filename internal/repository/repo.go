...
func GetSong(group, song, release_date, text string, limit, offset int) ([]models.Song, error) {
	...
	query := `SELECT group_name, song_name, release_date, text, link FROM songs WHERE 1=1`
	var args []interface{}
	...
	for rows.Next() {
		s := models.Song{}
		err = rows.Scan(&s.GroupName, &s.SongName, &s.ReleaseDate, &s.Text, &s.Link)
		if err != nil {
			return nil, err
		}
...
func GetDetailSong(id string) (*models.Song, error) {
	rows, err := db.DB.Query(`SELECT group_name, song_name, release_date, text, link FROM songs WHERE id=$1`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	if rows.Next() {
		if err = rows.Scan(&s.GroupName, &s.SongName, &s.ReleaseDate, &s.Text, &s.Link); err != nil {
			return nil, err
		}
	} else {
		return nil, err
...
func PostSong(s models.Song) error {
	_, err := db.DB.Exec(`INSERT INTO songs(group_name, song_name, release_date, text, link) VALUES ($1, $2, $3, $4, $5)`,
		s.GroupName, s.SongName, s.ReleaseDate, s.Text, s.Link)
	if err != nil {
		return err
	}
	return nil
...
func UpdateSong(id string, s models.Song) error {
	_, err := db.DB.Exec(`UPDATE songs SET group_name=$1, song_name=$2, release_date=$3, text=$4, link=$5 WHERE id=$6`,
		s.GroupName, s.SongName, s.ReleaseDate, s.Text, s.Link, id)
	if err != nil {
		return err
	}
	return nil
